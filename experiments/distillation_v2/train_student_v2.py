...
def main() -> None:
    ...
    tokenizer = train_bpe_tokenizer([row["text"] for row in train_rows], args.vocab_size, tokenizer_path)
    ...
    train_loader = DataLoader(
        BpeDistillDataset(train_rows, tokenizer, label_to_id, config.max_length),
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=0,
    )
    val_loader = DataLoader(
        BpeDistillDataset(val_rows, tokenizer, label_to_id, config.max_length),
        batch_size=args.eval_batch_size,
        shuffle=False,
        num_workers=0,
    )
    test_loader = DataLoader(
        BpeDistillDataset(test_rows, tokenizer, label_to_id, config.max_length),
        batch_size=args.eval_batch_size,
        shuffle=False,
        num_workers=0,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["macro_f1"] > best_macro_f1 + args.min_delta:
            ...
            best_epoch = epoch
            stale_epochs = 0
            save_checkpoint(output_dir / "model.pt", model, config, label_to_id, id_to_label, metadata)
        else:
            stale_epochs += 1
    ...
    best_model, config, id_to_label, checkpoint = load_checkpoint(output_dir / "model.pt", device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Distilled Student v2 with BPE and ensemble teachers.")
    parser.add_argument("--cache", default="experiments/distillation_v2/cache/ensemble_teacher_cache.jsonl")
    parser.add_argument("--output-dir", default="experiments/distillation_v2/runs")
    ...
    parser.add_argument("--hard-loss-weight", type=float, default=0.55)
    parser.add_argument("--soft-loss-weight", type=float, default=0.45)
    parser.add_argument("--personal-weight", type=float, default=1.45)
    parser.add_argument("--hard-weight-source", type=float, default=1.35)
...
def load_cache(path: Path, personal_weight: float, hard_weight: float) -> list[dict]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        source_weight = 1.0
        if source == "personal":
            source_weight = personal_weight
        elif source == "hard":
            source_weight = hard_weight
        rows.append(
            {
                "text": text,
                "label": label,
