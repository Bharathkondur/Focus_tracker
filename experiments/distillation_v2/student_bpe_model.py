...
class BpeDistillDataset(torch.utils.data.Dataset):
...
def load_checkpoint(path: str | Path, device: str | torch.device = "cpu") -> tuple[BpeIntentEncoder, StudentConfig, dict[int, str], dict]:
    ...
    model = BpeIntentEncoder(config, len(id_to_label))
    model.load_state_dict(checkpoint["model_state"])
    model.to(device)
    model.eval()
    return model, config, id_to_label, checkpoint
...
def predict(
    ...
    ids, mask = encode_text(tokenizer, text, config.max_length)
    ...
    return id_to_label[best_id], scores[id_to_label[best_id]], scores
